...
Player = Class {
    ...
    attack = function(self)
        if self.attackCoolDown <= 0 then
            self.attackDir = utils:toMouseVector(self.positionX, self.positionY)
            self:setDirFromVector(self.attackDir)

            effects:spawn("slice", self.positionX + self.collisionW / 2,
                          self.positionY + self.collisionH / 2, self.attackDir)
            self.state = 1
    ...
    swordDamage = function (self)
        local px, py = self.positionX + self.collisionW/2, self.positionY + self.collisionH/2;
